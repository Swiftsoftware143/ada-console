...
function parseHTML(html: string, url: string): ScanIssue[] {
...
const handler: Handler = async (event: HandlerEvent, context: HandlerContext) => {
  ...
  try {
    const { client_id, manual = false } = JSON.parse(event.body || '{}') as ScanRequest;
    ...
    await supabase
      ...
      .upsert({
        ...
        scan_count: supabase.rpc('increment_scan_count', { row_id: client_id })
