...
export const handler: Handler = async (event, context) => {
  ...
  try {
    ...
    for (const setting of (clients || []) as ClientScanSettings[]) {
...
async function scanClient(client: Client): Promise<ScanResult> {
...
async function emailReport(client: Client, scanResult: ScanResult): Promise<void> {
  if (!scanResult.success || !scanResult.scanId) return;
  ...
  const improvementText = scanResult.improvement !== null && scanResult.improvement !== undefined
