...
export const handler: Handler = async (event, context) => {
  ...
  try {
    ...
    const payload: FunnelSwiftPayload = JSON.parse(event.body || '{}');
    const { event: eventType, data, tracking_id } = payload;
...
async function createDemoAccount(data: DemoAccountData): Promise<DemoResult> {
  try {
    // Generate demo code
...
async function sendDemoWelcomeEmail({ to, firstName, demoCode, trialEndsAt, upgradeUrl }: EmailData): Promise<{ success: boolean }> {
  const emailHtml = `
    <h1>Welcome to ADASwift, ${firstName || 'there'}!</h1>
...
async function triggerWidgetAutomation(data: AutomationData): Promise<AutomationResult> {
  try {
    // Validate required fields
    const required = ['contact_email', 'business_name', 'domain'] as const;
    const missing = required.filter(f => !data[f as keyof AutomationData]);
...
async function sendWidgetEmail({ to, contactName, businessName, domain, planTier, embedCode, smtpSettings }: WidgetEmailData): Promise<{ success: boolean; messageId?: string; error?: string }> {
