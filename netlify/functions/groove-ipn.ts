...
const handler: Handler = async (event: HandlerEvent, context: HandlerContext) => {
  ...
  try {
    ...
    if (contentType.includes('application/json')) {
      ...
      data = Object.fromEntries(params) as unknown as GrooveData;
    }
...
// Send email via SMTP
async function sendEmail({ to, subject, htmlBody, smtpSettings }: EmailParams): Promise<EmailResult> {
  try {
...
function parsePlanTier(productName: string | undefined): string {
...
function parseBillingPeriod(productName: string | undefined): string {
