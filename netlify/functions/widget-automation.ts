...
const handler: Handler = async (event: HandlerEvent, context: HandlerContext) => {
  ...
  try {
    ...
    const required: (keyof WebhookData)[] = ['contact_email', 'business_name', 'domain'];
...
// Send email via SMTP
async function sendEmail({ to, contactName, businessName, domain, planTier, embedCode, smtpSettings }: EmailParams): Promise<EmailResult> {
  try {
...
function buildEmailTemplate({ contactName, businessName, domain, planTier, embedCode }: TemplateParams): string {
  return `<!DOCTYPE html>
<html>
<head>
  <style>
