...
const hydrate = (data: Client): Client => ({
  ...data,
...
export default function ClientDetail({ isPersonal = false }: ClientDetailProps) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setClient(hydrate(data as Client));
      loadTags();
      loadCdnDomain();
      setLoading(false);
    })();
  ...
  const update = useCallback(
    (patch: Partial<Client>) => setClient((c) => c ? ({ ...c, ...patch }) : null),
    []
  );
  ...
  const getCurrentTags = useCallback((): string[] => {
    ...
    try {
      ...
      if (newTags.length > 0) {
        ...
        try {
          ...
          let currentSettingsTags: string[] = [];
          if (settingsData?.value) {
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <Field label="Widget Position">
                <Select
                  ...
                  onValueChange={(v) => update({ widget_position: v as 'bottom-left' | 'bottom-right' })}
                >
                  <SelectTrigger
...
function Section({ title, subtitle, children, testId }: SectionProps) {
  return (
