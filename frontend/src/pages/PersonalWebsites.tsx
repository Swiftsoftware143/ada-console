...
function WebsitesEmptyState({ onAdd }: WebsitesEmptyStateProps): JSX.Element {
  return (
...
export default function PersonalWebsites(): JSX.Element {
  ...
  const [sortKey, setSortKey] = useState<keyof Website | 'tags' | 'active'>("created_at");
  ...
  const toggleActive = async (website: Website, e: MouseEvent): Promise<void> => {
    e.stopPropagation();
  ...
  const handleSort = (key: keyof Website | 'tags' | 'active'): void => {
    if (sortKey === key) {
  ...
  const filteredWebsites = useMemo(() => {
    return websites.filter((w) => {
      const matchesSearch =
        ...
        (typeof w.tags === 'string' && w.tags.toLowerCase().includes(search.toLowerCase())) ||
        w.location?.toLowerCase().includes(search.toLowerCase());
      
      // Check if website has the selected tag
      const websiteTags = w.tags ? (typeof w.tags === 'string' ? w.tags.split(',').map((t: string) => t.trim()) : w.tags) : [];
  ...
  const filteredSorted = useMemo(
    () => sortByKey(filteredWebsites, sortKey as string, sortDir),
            ...
            <tr className="text-left text-[10px] uppercase tracking-[0.15em] text-[#64748b]">
              {COLUMNS.map((col) => (
                <th key={col.key} className="px-6 py-3 font-bold">
                  <button
                    onClick={() => handleSort(col.key as keyof Website | 'tags' | 'active')}
                ...
                <td className="px-6 py-4">
                  {w.tags ? (
                    <div className="flex flex-wrap gap-1">
                      {(typeof w.tags === 'string' ? w.tags.split(',').map((t: string) => t.trim()) : w.tags).slice(0, 3).map((tag, i) => (
                        <span key={i} className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-[#007bff]/10 text-[#007bff] text-xs">
                          <Tag className="h-3 w-3" />
                          {tag}
                        </span>
                      ))}
                      {(typeof w.tags === 'string' ? w.tags.split(',').map((t: string) => t.trim()) : w.tags).length > 3 && (
                        <span className="text-[#64748b] text-xs">+{(typeof w.tags === 'string' ? w.tags.split(',').map((t: string) => t.trim()) : w.tags).length - 3} more</span>
                      )}
                    </div>
                  ) : (
